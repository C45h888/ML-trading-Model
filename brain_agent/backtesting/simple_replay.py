...
def _simulate_l2_update(coin: str, last_price: float) -> L2Update:
    """
    Simulate L2 orderbook update with random small perturbations around last price.
    ...
    bids = []
    for i in range(5):
        px = last_price - (i + 1) * random.uniform(0.5, 2.0)
        sz = random.uniform(1.0, 10.0)
...
async def run_backtest(
    ...
    """
    logger.info(f"Starting backtest: coin={coin}, limit={limit}")
    ...
    for i, trade in enumerate(trades):
        ...
        # Simulate L2 updates periodically (every 10 trades)
        if (i + 1) % 10 == 0 and trade.price > 0:
            l2 = _simulate_l2_update(coin, trade.price)
            l2_dict = {
